...
/* REPRL command packet */
#define REPRL_RESP_MASK 0x80
// [1 byte opcode][4 bytes length][n bytes payload]
typedef struct {
    uint8_t opcode;
    uint32_t length;
} __attribute__ ((packed)) reprl_cmd_pkt_t;
...
// TODO: review these
#define REPRL_MAX_ARGV_SIZE 0x100
#define REPRL_MAX_ENVP_SIZE 0x100
typedef struct {
    uint16_t ctx_handle;
...
/* TODO */
/* Perhaps coverage command packet */
/* Perhaps coverage data packet */
